...
def train_opt(args, seed):
    ...
    train_utils.adjust_seed_weights(model, seed)
    ...
    with open("./reporting/" + "new_sgd_default_c_" + str(args.c) + "_p_" + str(args.p) + "_graph_" + args.graph_mode +
              "_dataset_" + args.dataset_mode + "_seed_" + str(args.seed) + "_name_" + args.name + ".csv", "w") as f:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if max_test_acc < test_acc:
                ...
                if not os.path.isdir('checkpoint'):
                    os.mkdir('checkpoint')
                filename = "new_sgd_default_c_" + str(args.c) + "_p_" + str(args.p) + "_graph_" + args.graph_mode + \
...
def visualize_net(args, seed, writer):
    trainloader, test_loader = load_data(args)
...
def main():
    ...
    writer = SummaryWriter('runs/CIFAR10_first_gen')

    for network in range(1, 11):
        visualize_net(args, network, writer)
