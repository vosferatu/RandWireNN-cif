...
def run_predicted_eval(model, test_loader):
    ...
    train_utils.adjust_edge_weights(model, new_weights)
...
def check_node_weights(model):
    i = 0
    for graph_conv in model.children():
        if type(graph_conv[0]) == randwire.RandWire:
            for node in graph_conv[0].module_list:
                print(len(node.unit.unit[1].conv.weight[0][0]))
                print(node.unit.unit[1].pointwise.weight[0][0][0][0].item())
    print('TOTAL: ', i, 'per rand: ', i / 3)
...
def main():
    ...
    parser.add_argument('--seed', type=int, default=3, help='seed in the random graph algorithm')
    ...
    check_node_weights(model)

    train_utils.get_node_dataset(model)
